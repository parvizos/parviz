...
export async function seedDemo(db: DrizzleDb): Promise<void> {
  ...
  /* ── Конспекты ── */
  const noteBodies = [
    "<h2>Предел по Коши</h2><p>Для любого ε&gt;0 найдётся δ&gt;0, что при |x−a|&lt;δ верно |f(x)−L|&lt;ε.</p><div data-callout data-variant=\"info\"><p>Главное: δ зависит от ε — чем меньше ε, тем меньше δ.</p></div><h2>Замечательные пределы</h2><ul><li>lim (sin x)/x = 1 при x→0</li><li>lim (1 + 1/n)ⁿ = e</li></ul><div data-callout data-variant=\"warn\"><p>На экзамене любят неопределённость 0/0 — сразу вспоминай правило Лопиталя.</p></div><blockquote>Спросить у препода про Лопиталя на семинаре в среду.</blockquote>",
